...
class PVTv2B2Encoder(nn.Module):
    """PVTv2-B2 特征提取（需要 timm，默认加载 ImageNet-1K 预训练权重）。"""

    def __init__(self, in_channels: int = 2, pretrained: bool = True):
        super().__init__()
        if timm is None:
...
class ACANet(nn.Module):
    ...
    def __init__(self, config: ACANetConfig):
        ...
        if config.use_pvt:
            ...
            self.encoder_up = PVTv2B2Encoder(in_channels=2, pretrained=config.pretrained_backbone)
            self.encoder_down = PVTv2B2Encoder(in_channels=2, pretrained=config.pretrained_backbone)
