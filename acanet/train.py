...
CONFIG_BLOCK = {
    ...
    "pvt_weights_path": "pvt_v2_b2.pth",
...
def fit(config: TrainingConfig) -> Dict[str, float]:
    ...
    for epoch in range(config.num_epochs):
        ...
        lr_factor = (1 - (epoch + 1) / config.num_epochs) ** config.poly_power
...
def main() -> None:  # pragma: no cover - CLI 入口
    ...
    model_cfg = ACANetConfig(
        ...
        pretrained_backbone=cfg.get("pvt_weights_path") is None and cfg.get("pretrained_backbone", False) is True,
    )
