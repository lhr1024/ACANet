...
def save_prediction_masks(
    preds: List[torch.Tensor], out_dir: Path, base_names: List[str]
) -> None:
    """保存预测掩码为 .npy，形状 (H,W,3) 对应 WT/TC/ET。"""
    out_dir.mkdir(parents=True, exist_ok=True)
    if len(preds) != len(base_names):
        raise ValueError("Prediction count does not match base name count.")
    for idx, (pred, base_name) in enumerate(zip(preds, base_names)):
        # pred: (C,H,W)
        if pred.numel() == 0:
        ...
        save_path = out_dir / f"{base_name}.npy"
...
def main() -> None:
    ...
    base_names = [Path(p).stem for p in img_files_npy[:n]]
    save_prediction_masks(preds[:n], result_dir, base_names)
