...
def calculate_brats_metrics(gt_dir: Path, pred_dir: Path) -> None:
    ...
    for gt_path, pred_path in tqdm(zip(gt_paths, pred_paths), total=len(gt_paths), desc="Calculating metrics"):
        ...
        dice_wt_sum += dice_coefficient(pred_wt, gt_wt)
        dice_tc_sum += dice_coefficient(pred_tc, gt_tc)
        dice_et_sum += dice_coefficient(pred_et, gt_et)

        iou_wt_sum += iou_score(pred_wt, gt_wt)
        iou_tc_sum += iou_score(pred_tc, gt_tc)
        iou_et_sum += iou_score(pred_et, gt_et)
